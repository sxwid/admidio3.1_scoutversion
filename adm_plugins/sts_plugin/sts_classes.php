<?php
/******************************************************************************
 * Startseite zusätzliche Klassen
 *
...
$tablename=$g_tbl_praefix.'_user_sts';
define("TBL_USER_STS",$tablename);
unset($tablename);
...
{
    global $gDb;
    $sql_select="SHOW TABLES LIKE '".TBL_USER_STS."'"; 
 ...
 */
class ModuleSts extends Modules
{
    ...
    public function getDataSet($startElement = 0, $limit = NULL)
    {
        ...
        {
            $this->getConditions = 'sts_id ='. $this->getParameter('id');
        }
        // TODO only working global, could add for each organisation   
        //read sts from database
        $sql = 'SELECT * FROM '. TBL_USER_STS. ' WHERE '.$this->getConditions.'';
        $result = $gDb->query($sql);

        //array for results       
        $sts= array('numResults'=>$gDb->num_rows($result), 'limit' => $limit, 'totalCount'=>$this->getDataSetCount());
        ...
        $sts['parameter'] = $this->getParameters();
        return $sts;
    }  
}

class TableSts extends TableAccess
{    
    ...
    {
        if($columnName == 'sts_description')
        {
            if(isset($this->dbColumns['sts_description']) == false)
            {
                $value = '';
            }
            ...
            {
                $value = html_entity_decode(strStripTags($this->dbColumns['sts_description']), ENT_QUOTES, 'UTF-8');
            }
            else
            {
                $value = $this->dbColumns['sts_description'];
            }
        }
        else
        {
    ...
    public function save($updateFingerPrint = true)
    {
        ...
        if($this->new_record)
        {
            $this->setValue('sts_org_shortname', $gCurrentOrganization->getValue('org_shortname'));
        }
